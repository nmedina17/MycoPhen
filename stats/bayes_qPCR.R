...
# plot(qPCRfullQC_EM_QUBI1$copies_log)
...
qPCRfullQC_EM_QUBI1_list <- qPCRfullQC_EM_QUBI1 %>% 
  ...
  # as.data.frame() %>% 
  as.list()
l <- list(T = qPCRfullQC_EM_QUBI$SESSION %>% unique() %>% length(), 
          X = qPCRfullQC_EM_QUBI$Moisture1_7d_avg, 
          X_1 = qPCRfullQC_EM_QUBI$Temp1_7d_avg,
...
# figures----
## Y
qua <- apply(sample_stan$y, 2, quantile,
             probs=c(2.5, 25, 50, 75,97.5)/100, na.rm = T) %>%
  t() %>% as.data.frame()

ggplot() +
  geom_point(data = d, aes(x=c(1:T), y=Y)) +
  geom_line(aes(x=c(1:T), y=qua$`50%`)) + #median
  #BayesCriticalInterval(CI)
  geom_ribbon(data=qua, aes(x=c(1:T), ymin = `2.5%`, ymax = `97.5%`), 
              alpha = 0.2)
